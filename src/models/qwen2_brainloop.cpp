...
llm_build_qwen2_brainloop::llm_build_qwen2_brainloop(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (cache.cart_v && il >= 30 && il <= 33) {
            ...
            // Matmul broadcast: cart^T [1,embd] * ones [1,nt] -> [embd,nt]
            int nt = (int)cur->ne[1];
            ggml_tensor * cart_T = ggml_reshape_2d(ctx0, cart_proj, 1, n_embd_i);
            struct ggml_init_params op = { ggml_tensor_overhead() + nt*sizeof(float), nullptr, false };
            struct ggml_context * octx = ggml_init(op);
